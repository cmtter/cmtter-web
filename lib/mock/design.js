...
export async function codeGen(req, res) {
  const { defContent, pageContent, file } = req.body
  try {
    const file1 = path.resolve(__dirname, '../../views/-' + file + '-define.jsx')
    const file2 = path.resolve(__dirname, '../../views/' + file + '.vue')
    jetpack.write(file1, defContent)
    jetpack.write(file2, pageContent)
    res.json({
      isOk: true
    })
  } catch (error) {
