 ...
 * 2. > 10条会自动显示搜索
 */
 import { defineComponent, computed, ref, inject, watch, shallowRef, toRaw, cloneVNode,isVNode} from 'vue'
 import classNames from 'ant-design-vue/es/_util/classNames'
 import { getOptionProps } from 'ant-design-vue/es/_util/props-util'
 import VueTypes from  'vue-types'
 import UIConfig from './ui-config'
 import { defalutProps } from '../utils'
 import { Select, Form, spin, Row, Col } from 'ant-design-vue'
 ...
 function generate(options){
   ...
   const _formControl = {
     ...
     render(){
       ...
       let addonAfter = null
       if (this.$slots && this.$slots.addonAfter){
         addonAfter = this.$slots.addonAfter()
         if(addonAfter && isVNode(addonAfter)){
           addonAfter = cloneVNode(addonAfter, {
             style: {
       ...
       const content = this.onlyRenderControl ? (<div class={_class} style="height: 100%"> {formController} </div>) : (<FormItem {...formItemProps} class={_class}>
         <Row type="flex">
           <Col flex="auto">{formController}</Col>
           {
