...
export default async function (cwd, options) {
  ...
  let customerRouters = await globby(['**/*.*(jsx|vue)'], {
    cwd: rootPath,
    ignore: ['**/components/*.*(jsx|vue)', '**/-*.*(jsx|vue)', '**/_*.*(jsx|vue)']
  })
  ...
  customerRouters = customerRouters.map(routeFile => {
    ...
    //组件
    const componentPath = resolve(rootPath, routeFile).replace(/\\/g, '/')
    ...
    return `
      {
        ...
        component: ${component},
        meta: {title: \"${routeName.replace(/-/g, '&')}\", closable: ${routeName === dashboard.name ? 'false' : 'true'}}
      }
    `
  })
