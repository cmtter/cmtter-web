...
import resolveRouterFromFile from './asd/resolve-router-from-file'
 ...
 * @returns 
 */
function getTitle(title, pageRouterMeta) {

  if (pageRouterMeta && pageRouterMeta.title) {
    return pageRouterMeta.title
  }
  if (title === 'devopts') {
    return '页面设计'
  }
...
export default async function (cwd, options) {
  ...
  const pageRouterMetaAllPromises = customerRouters.map((routeFile, index) => {
    const componentPath = resolve((index >= docViewIndex ? docViewRootPath : rootPath), routeFile)
    return resolveRouterFromFile(componentPath)
  })
  const pageRouterMetas = await Promise.all(pageRouterMetaAllPromises)
  const pageRouterMetasMap = pageRouterMetas.reduce((m, metaItem) => {
    m[metaItem.file] = metaItem.meta
    return m
  }, {})
  ...
  customerRouters = customerRouters.map((routeFile, index) => {
    ...
    //组件
    let componentPath = resolve((index >= docViewIndex ? docViewRootPath : rootPath), routeFile).replace(/\\/g, '/')
    const pageRouterMeta = pageRouterMetasMap[resolve(componentPath)] || {}
    ...
    return `
      {
        ...
        component: ${component},
        meta: {title: \"${getTitle(routeName.replace(/-/g, '&'), pageRouterMeta)}\", closable: ${routeName === dashboard.name ? 'false' : 'true'}}
      }
    `
  })
