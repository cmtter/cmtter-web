...
module.exports = async function (glob, metaData, globOptions, dest, cleaable = false) {
  ...
  _files.forEach((file) => {
    ...
    if (globOptions.ingorFn && globOptions.ingorFn(file, templatePath) === false) {
      jetpack.write(path.resolve(tempPath, file), content)
    } else {
      jetpack.write(path.resolve(tempPath, file), template.render(content, metaData, { escape: false, compileDebug: true }))
    }
